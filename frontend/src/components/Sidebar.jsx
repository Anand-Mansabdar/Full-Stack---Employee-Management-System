...
const Sidebar = () => {
  ...
  // Close the mobile menu sidebar on route change
  useEffect(() => {
    setMobileOpen(false);
  }, [pathname]);
        ...
        <div className="mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-medium text-slate-200 truncate tracking-wider">
                {userName}
              </p>
              <p className="text-[11px] text-slate-500 truncate font-semibold tracking-wide">
  ...
  return (
    <>
      ...
      <button
        className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-neutral-900 text-white rounded-lg shadow-lg border border-white/10"
      ...
      {/* Sidebar on mobile */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-neutral-900 via-neutral-900 to-neutral-950 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebarContent}
      </aside>
    </>
